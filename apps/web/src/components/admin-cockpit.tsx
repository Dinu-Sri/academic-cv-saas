...
function InvitesPanel() {
  ...
  async function createInvite() {
    ...
    try {
      const response = await fetch("/api/admin/invites", {
        ...
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, planKey, expiresInDays, note, sendEmail })
      });
      ...
      setLastUrl(payload.redeemUrl || "");
      setMessage(
        payload.email?.sent
          ? "Invitation created and email sent."
          : `Invitation created${payload.email?.reason ? ` (email: ${payload.email.reason})` : ""}. Copy the link below.`
      );
      setEmail("");
      setNote("");
      await loadInvites();
  ...
  return (
    <section className="admin-invites-panel">
      <h3>Package invitations</h3>
      <p className="muted-text">
        Create a single-use link bound to one email. The recipient must sign in with that email to activate PDF Pass or Scholar Annual.
      </p>
      <div className="admin-invite-form">
        <label>
          <span>Email</span>
          <input value={email} onChange={(event) => setEmail(event.target.value)} type="email" placeholder="scholar@university.edu" />
        </label>
        <label>
          <span>Plan</span>
        ...
        <label>
          ...
          <input value={note} onChange={(event) => setNote(event.target.value)} placeholder="Conference grant, pilot, etc." />
        </label>
        <label className="admin-invite-check">
          <input type="checkbox" checked={sendEmail} onChange={(event) => setSendEmail(event.target.checked)} />
          <span>Email the invitation link</span>
        </label>
        <button className="primary-action compact-action" type="button" disabled={busy || !email.trim()} onClick={() => void createInvite()}>
