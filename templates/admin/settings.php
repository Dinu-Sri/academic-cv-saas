...
<!-- PDF Render Engine Configuration -->
<div class="container pb-4">
    <div class="row g-4">
        <div class="col-lg-8">
            <div class="card shadow-sm">
                <div class="card-header bg-white py-3">
                    <h5 class="mb-0 fw-bold"><i class="bi bi-file-earmark-pdf me-2"></i>PDF Render Engine</h5>
                </div>
                <div class="card-body p-4">
                    <form method="POST" action="<?= APP_URL ?>/admin/settings/update">
                        <?= Auth::csrfField() ?>
                        <input type="hidden" name="_form" value="pdf_engine">

                        <div class="alert alert-warning small mb-4">
                            <i class="bi bi-exclamation-triangle-fill me-1"></i>
                            <strong>Heads up:</strong> Switching the default to <code>latex</code> requires the
                            <code>cvscholar:latex</code> Docker image to be deployed. If <code>xelatex</code> is missing
                            at runtime, every render automatically falls back to FPDF (no user impact), but you'll see
                            <code>fallback=1</code> rows in metrics.
                        </div>

                        <div class="mb-4">
                            <label class="form-label fw-semibold">Default Engine</label>
                            <select class="form-select" name="pdf_engine_default">
                                <option value="fpdf" <?= ($settings['pdf_engine_default'] ?? 'fpdf') === 'fpdf' ? 'selected' : '' ?>>FPDF (current production engine, safe default)</option>
                                <option value="latex" <?= ($settings['pdf_engine_default'] ?? 'fpdf') === 'latex' ? 'selected' : '' ?>>xelatex (real LaTeX, requires cvscholar:latex image)</option>
                            </select>
                            <div class="form-text">Used when no template- or user-level override applies.</div>
                        </div>

                        <div class="form-check form-switch mb-3">
                            <input class="form-check-input" type="checkbox" id="pdf_engine_template_override" name="pdf_engine_template_override" value="1"
                                <?= ($settings['pdf_engine_template_override'] ?? '1') === '1' ? 'checked' : '' ?>>
                            <label class="form-check-label fw-semibold" for="pdf_engine_template_override">
                                Honor per-template engine overrides
                            </label>
                            <div class="form-text">When ON, <code>templates.style_config.engine</code> is respected. Recommended for piloting xelatex on one template at a time.</div>
                        </div>

                        <div class="form-check form-switch mb-4">
                            <input class="form-check-input" type="checkbox" id="pdf_engine_user_override" name="pdf_engine_user_override" value="1"
                                <?= ($settings['pdf_engine_user_override'] ?? '0') === '1' ? 'checked' : '' ?>>
                            <label class="form-check-label fw-semibold" for="pdf_engine_user_override">
                                Honor per-user engine preference
                            </label>
                            <div class="form-text">When ON, <code>users.cv_settings.preferred_pdf_engine</code> takes priority over template/site default.</div>
                        </div>

                        <div class="alert alert-info small mb-0">
                            <i class="bi bi-shield-check me-1"></i>
                            <strong>Circuit breaker is always on:</strong> any engine with &gt;2% failure rate over the
                            last hour (≥10 samples) is automatically downgraded to FPDF until the rate drops.
                            No flag flip needed.
                        </div>

                        <div class="mt-3 d-flex flex-wrap gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-check-lg me-1"></i>Save Engine Settings
                            </button>
                        </div>
                    </form>
                </div>
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm">
                <div class="card-body">
                    <h6 class="fw-bold mb-3"><i class="bi bi-bar-chart me-1"></i>Last 24h Renders</h6>
                    <?php if (empty($pdfEngineStats)): ?>
                        <p class="text-muted small mb-0">No render events yet. Stats will appear here after users compile a CV.</p>
                    <?php else: ?>
                        <table class="table table-sm small mb-0">
                            <thead>
                                <tr><th>Engine</th><th class="text-end">Total</th><th class="text-end">Fail</th><th class="text-end">FB</th><th class="text-end">ms</th></tr>
                            </thead>
                            <tbody>
                                <?php foreach ($pdfEngineStats as $row): ?>
                                <tr>
                                    <td><code><?= e($row['engine']) ?></code></td>
                                    <td class="text-end"><?= (int) $row['total'] ?></td>
                                    <td class="text-end <?= ((int)$row['failures']) > 0 ? 'text-danger fw-semibold' : 'text-muted' ?>"><?= (int) $row['failures'] ?></td>
                                    <td class="text-end <?= ((int)$row['fallbacks']) > 0 ? 'text-warning fw-semibold' : 'text-muted' ?>"><?= (int) $row['fallbacks'] ?></td>
                                    <td class="text-end text-muted"><?= (int) $row['avg_ms'] ?></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                        <div class="form-text mt-2 small">FB = fell back to FPDF after primary failed.</div>
                    <?php endif; ?>
                </div>
            </div>
            <div class="card shadow-sm mt-3">
                <div class="card-body">
                    <h6 class="fw-bold mb-2"><i class="bi bi-lightbulb me-1"></i>Pilot Tip</h6>
                    <p class="small text-muted mb-0">
                        To test xelatex on a single template without changing the global default, run:
                        <code class="d-block mt-2">UPDATE templates SET style_config = JSON_SET(COALESCE(style_config, '{}'), '$.engine', 'latex') WHERE id = 1;</code>
                    </p>
                </div>
            </div>
        </div>
