...
22. **Email provider (rewrite)**: all transactional mail goes through `apps/web/src/lib/email` (`sendTransactionalEmail`). Prefer **Brevo** (`BREVO_API_KEY`, `EMAIL_PROVIDER=auto|brevo`). Resend remains a fallback (`RESEND_API_KEY`). Optional contact sync: `BREVO_ALL_USERS_LIST_ID`, `BREVO_MARKETING_LIST_ID`, `BREVO_CONTACT_SYNC_ENABLED=1` — signup upserts contacts; Settings → marketing opt-in adds/removes the marketing list. Product events still trigger sends in code (hybrid); Brevo Automation can use list membership for marketing drips.
