...
| `apps/web/src/lib/meta/` | Meta Pixel + Conversions API (config, browser, CAPI, consent, track helpers) |
| `apps/web/src/components/meta-pixel.tsx` | Browser Pixel base snippet (main product host only) |
| `docs/META_ADS_TRACKING.md` | Ads event catalog, campaign recipes, env vars, ops checklist |
| `apps/web/src/lib/mobile/` | Phone detection, preference cookie, mobile compile helper |
| `apps/web/src/app/m/` | Minimal mobile start/upload/manual/ready flow |
| `docs/MOBILE_UX.md` | Mobile UX product rules, routes, gate, handoff |
...
| `apps/web/src/lib/academic-field-suggestions.ts` | Deduplicated retention of user-supplied specific fields for later taxonomy review |
...
| `apps/web/prisma/migrations/202607290003_academic_field_suggestions/` | Custom academic-field suggestion records, unique per profile and normalized field key |
...
12. **Google login (rewrite)**: set `GOOGLE_CLIENT_ID` + `GOOGLE_CLIENT_SECRET` in Portainer (`docker-compose.rewrite.yml`). Callback URL must allow `{BETTER_AUTH_URL}/api/auth/callback/google`. Password reset emails need `EMAIL_FROM` plus `BREVO_API_KEY` (preferred) or `RESEND_API_KEY`.
...
18. **Meta Pixel + CAPI (paid ads)**: set `META_TRACKING_ENABLED=1`, `NEXT_PUBLIC_META_PIXEL_ID`, `META_CAPI_ACCESS_TOKEN` (and optional `META_CAPI_PIXEL_ID`, `META_CAPI_TEST_EVENT_CODE`) in Portainer rewrite compose. Dual-path events use shared `event_id`. Optimize sales for `Purchase`, growth for `CompleteRegistration`. Never send CV content; never load Pixel on scholar public sites. Free invite/admin grants must not fire `Purchase` (use `InviteRedeemed`). See `docs/META_ADS_TRACKING.md`.
20. **Corporate invoices (rewrite)**: A4 HTML invoices from `/api/billing/invoice/[orderId]` use `lib/billing/company.ts` + `invoice-document.ts` (Clossyan Technologies (Pvt) Ltd / CVScholar). Invoice No. = PayHere/order ID. VAT not charged. Admin grants create `$0` complimentary invoices shown in the billing sidebar. Checkout supports discount codes; admin CRUD at `/admin/discount-codes` (Prisma `discount_codes`). Homepage hero YouTube embed stays paused (`DEMO_VIDEO_EMBED=""`) until a real walkthrough URL is set in `home-landing.tsx`.
21. **Microsoft Clarity (rewrite)**: set `CLARITY_ENABLED=1` and `NEXT_PUBLIC_CLARITY_PROJECT_ID` in Portainer rewrite compose. Observes product public pages (home, pricing, blog, app) and, by default, published scholar sites (`CLARITY_PUBLIC_SITES_ENABLED=1`). Unlike Meta Pixel, Clarity is allowed on public scholar hosts for UX observation. Kill switch: `CLARITY_ENABLED=0`.
22. **Email provider (rewrite)**: all transactional mail goes through `apps/web/src/lib/email` (`sendTransactionalEmail`). Prefer **Brevo** (`BREVO_API_KEY`, `EMAIL_PROVIDER=auto|brevo`). Resend remains a fallback (`RESEND_API_KEY`). Contact sync: `BREVO_ALL_USERS_LIST_ID`, `BREVO_MARKETING_LIST_ID`, `BREVO_CONTACT_SYNC_ENABLED=1`. Signup defaults **marketing on** (checkbox + Google); Settings opt-out removes marketing list. Guest homepage popup (`MarketingCapturePopup` → `/api/marketing/subscribe`) adds emails to the marketing list without creating an account. HTML templates live in `lib/email/templates/` (minimal layout + logo). Admin test console: `/admin/emails`.
23. **SEO (rewrite product host)**: `/sitemap.xml` (`app/sitemap.ts`), `/robots.txt` (`app/robots.ts`), `/llms.txt` (`app/llms.txt/route.ts`). JSON-LD helpers in `lib/seo/platform.ts` (Organization, WebSite, SoftwareApplication, WebPage, BlogPosting, CollectionPage, BreadcrumbList, FAQPage). Marketing pages (home, pricing, blog, legal, methodology) emit schema; published scholar sites keep per-site sitemap/robots/llms under `/u/{username}/…`.
19. **Mobile minimal flow**: phone User-Agents hitting power routes (`/profile`, `/cv`, `/website`, …) redirect to `/m` unless cookie `cvscholar_mobile_mode=full` or `CVSCHOLAR_MOBILE_FLOW_ENABLED=0`. Guest-first upload/manual → ready handoff; full editor stays desktop. See `docs/MOBILE_UX.md`.
...
17. **Logged-in CV compilation requires structured academic identity**: country, major academic field, and specific academic field are enforced in the editor and `/api/cv/compile`; guests retain the lighter trial compile flow. Custom specific fields are retained through `academic-field-suggestions.ts` for taxonomy review.
